...
## 10. Fix: el workflow tardaba varios minutos

`PATCH /drills/bulk` volvia a pedir y recorrer TODAS las tarjetas del sitio
(getNodesWithType + caminar el arbol de ancestros para cada una) por CADA
actualizacion individual -- para 20 actualizaciones sobre ~60 instancias
(20 tarjetas x 3 breakpoints), eso son miles de idas y vueltas a Framer.

`bulkUpdateDrills()` en `src/framer-drills.ts` lo hace UNA sola vez: trae y
recorre todas las instancias al principio, y despues solo hace las llamadas
de escritura reales (setAttributes) para las que efectivamente cambian.
`/drills/bulk` ahora usa esta funcion en vez de llamar a `updateDrillGroup`
en un loop.

## 11. Fix: "Drill not found" con datos que parecian viejos

El emparejamiento por `program/day/block/currentExercise` (texto) resulto
fragil: `exercise` es justo el campo que estamos cambiando todo el tiempo,
asi que un valor leido por `GET /drills` podia quedar desactualizado para
el momento en que `/drills/bulk` intentaba usarlo para encontrar la
tarjeta, sobre todo probando varias corridas seguidas rapido.

Ahora cada tarjeta se identifica por un **id estable** (`DrillGroup.id`,
el id de su copia Desktop) que no cambia sin importar que texto tenga la
tarjeta. `GET /drills` ya devuelve este campo `id` en cada drill. El Code
node de `randomize-from-sheet-workflow.json` ahora arma cada actualizacion
como `{ id, exercise, duration, coachNotes, purpose }` en vez de usar
`currentExercise` como identificador, y `PATCH /drills/bulk` matchea
exclusivamente por ese `id`.
