...
export async function bulkUpdateDrills(
  ...
  const withCards: Array<{ instance: ComponentInstanceNode; card: RawDrillCard }> = [];
  for (const instance of drillInstances) {
    const breadcrumb = await getAncestorNames(instance);
    withCards.push({ instance, card: toRawDrillCard(instance, breadcrumb) });
  }
  ...
  for (const u of updates) {
    ...
    // already have.
    const matches = withCards.filter(
      (wc) =>
        wc.card.program === program &&
        wc.card.day === day &&
        wc.card.block === block &&
        wc.card.exercise === currentExercise,
    );
    ...
    results.push({
      drill: {
        ...
        exercise: cleanUpdate.exercise ?? currentExercise,
